-- =============================================================================
-- 010 — la copia de un escenario no pierde de que resumen salio cada pago
-- =============================================================================
--
-- copy_scenario dejaba `statement_id` y `absorbed_by_statement_id` en null en
-- los pagos copiados, por analogia con lo que ya hacia con
-- expenses.archived_by_statement_id: el flag es lo que decide la cuenta y los
-- ids apuntan a filas del escenario viejo.
--
-- Con los gastos eso alcanza, porque is_archived se basta solo. Con los pagos
-- no, y la diferencia cuesta plata: saveStatement corrige el pago de un
-- resumen buscandolo por `statement_id`. Sin ese id, volver a guardar el
-- resumen en la copia no encuentra el pago viejo y agrega otro. Quedan dos
-- pagos por el mismo resumen y el saldo baja dos veces, en silencio.
--
-- Apareció corriendo copy_scenario contra la base por primera vez: la funcion
-- se habia tocado dos veces el dia anterior sin ejecutarla nunca.
--
-- El arreglo es de orden: los resumenes se copian ANTES que los pagos, y cada
-- pago se reengancha al resumen copiado. No hace falta un mapa aparte como el
-- de deudas — (debt_id, period) es unico en card_statements, asi que con la
-- deuda copiada y el periodo se llega solo.
