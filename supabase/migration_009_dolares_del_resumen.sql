-- =============================================================================
-- 009 — los dolares del resumen cuentan
-- =============================================================================
--
-- El resumen declara un total en dolares y la app lo dejaba afuera del saldo,
-- con un aviso que decia "cargalos a mano si queres que cuenten". Nadie sabia
-- donde. El resultado era plata real escondida: la tarjeta debia mas de lo que
-- la app mostraba, y ninguna pantalla lo decia.
--
-- Se dejaba afuera por una razon buena: los dolares se pagan a la cotizacion
-- del cierre y esa cotizacion NO esta en el PDF. Convertirlos con un numero
-- inventado por nosotros hubiera sido peor que no sumarlos.
--
-- La salida es pedirla. El formulario del resumen tiene ahora el total en
-- dolares (que si sale del PDF) y la cotizacion (que la pone la persona). Con
-- las dos, el equivalente en pesos entra al cierre como un consumo mas y el
-- preview lo nombra aparte. Sin cotizacion, los dolares siguen sin entrar y el
-- aviso ahora dice exactamente que falta.
--
-- Se guardan las dos cifras y no solo el resultado: el peso equivalente ya
-- quedo adentro del cierre, asi que sin la cotizacion no habria forma de
-- explicar de donde salio.

alter table card_statements
  add column if not exists usd_balance numeric(14, 2) not null default 0;
alter table card_statements
  add column if not exists usd_rate numeric(14, 4);

-- -----------------------------------------------------------------------------
-- copy_scenario, al dia con las dos columnas
-- -----------------------------------------------------------------------------
--
-- Copiada tal cual de schema.sql, no reescrita de memoria.
